...
export const getListAccPaymentSetupApi = async (
  search = "",
  page = 0,
  limit = 0
) => {
  try {
    ...
    const response = await requestApi({
      url: `${process.env.NEXT_PUBLIC_REST_API_URL}/acc-payment-setup${param}`,
      method: "GET",
      isAuth: true,
    });
...
export const getDetailAccPaymentSetupApi = async (id) => {
  try {
    const response = await requestApi({
      url: `${process.env.NEXT_PUBLIC_REST_API_URL}/acc-payment-setup/${id}`,
      method: "GET",
      isAuth: true,
    });
    return response;
