  ...
  return (
    <React.Fragment>
      ...
      </TableRow>
      {
        ...
        //           </TableHead>
        //           <TableBody>
        //             {/* {props?.row?.order_quote_data[0]?.map((item, key) => (
        //               <TableRow key={key}>
        //                 <TableCell>{item?.unit_code}</TableCell>
        //                 <TableCell>{item?.container_type_data}</TableCell>
        //               </TableRow>
        //             ))} */}
...
export default function Page() {
  ...
  useEffect(async () => {
    try {
      setOpen(true);
      var data = await getListQuoteInStatusApi("production");
      // var data = await getListQuoteInApi();
      setRowCount(data.total);
      setListData(data.result);
      setPage(0);
  ...
  const buildForm = (e, id) => {
    openPage(e, getRoute("order-production", { id: id }));
  };
      ...
      <Box className="p-5 content-wrapper">
        ...
        >
          <Grid item xs={12} lg={12} xl={12}>
            <h1 className="mb-3">Container in Production</h1>
