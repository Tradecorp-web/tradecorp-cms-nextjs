import { Button, Card, Grid, MenuItem, Modal, Typography, Box, TextField, Paper, List, ListItem, ListItemText, ListItemIcon, Checkbox, makeStyles, Backdrop, CircularProgress, IconButton, InputAdornment, FormControl, InputLabel, Input, Select, Chip, LinearProgress } from "@material-ui/core"
import Autocomplete from '@material-ui/lab/Autocomplete'
import { Delete, Add } from '@material-ui/icons'
import React, { useEffect, useState } from "react"
import { saveDocumentApi } from "../../../services/api/document.api"
import { saveMultiFolderApi,purgeFolderApi } from "../../../services/api/folder.api"
import { FileUploadComponent3 } from "../../base_component/file-upload"
import { DropzoneArea } from "material-ui-dropzone"
import { uploadFileDocApi, deleteFileApi } from "../../../services/api/file.api"
...
export default function DocumentForm(props) {
    ...
    const [errorText, setErrorText] = useState({document_name:null,link:null})
    const [isLoading, setLoading] = useState(false)

    const [folder, setFolder] = useState("")
    const [data, setData] = useState({id:null,document_name:null,folder_id:null,upload_date:null,link:null})
    ...
    const uploadFile = (file,len,folder_id="") => {
        uploadFileDocApi(folder,file,(event) => {
            var persen = Math.round((event.loaded/event.total)*100)
            for (var i=0; i<onGoing.length; i++) {
                if (onGoing[i].name == file.name) 
                {
                    onGoing[i].progress = persen
            ...
            if (folder_id == "") {
                procList.push({...data, link:response.link, content_type:response.content_type, document_name:response.file_name})
            } else {
                procList.push({...data, folder_id:folder_id, link:response.link, content_type:response.content_type, document_name:response.file_name})
            }
            countUp++
            if (countUp == len) {
            ...
            for (var i=0; i<onGoing.length; i++) {
                if (onGoing[i].name == response.file_name) {
                    onGoing.splice(i,1)
                }
            }
    ...
    const processListFile = (files,no) => {
        ...
        if (section.length > 1) {
            ...
            var stack = {folder_stack:folderStack,parent_id:data.folder_id,parent_tree:props.doc.folder,teams:props.doc.teams,users:props.doc.users}
            saveMultiFolderApi(stack).then((res) => {
    ...
    const uploadFileProccess = (files) => {
        ...
        if (files.length > 0) {
            for (var i=0;i<files.length;i++) {
                fileList.push({name:files[i].name,progress:0})
            }
            onGoing = [...fileList]
    ...
    useEffect(() => {
        if (props.doc != null) {
            ...
            if (props.doc.id != null) {
                setData({id:props.doc.id,document_name:props.doc.document_name,folder_id:props.doc.folder_id,upload_date:props.doc.upload_date,link:props.doc.link})
                ...
                setTitle({formTitle:"Edit Document",buttonTitle:"Save"})
            } else {
                ...
                setTitle({formTitle:"Add New Document",buttonTitle:"Save"})
            }
        } else {
            setData({id:null,document_name:null,folder_id:null,upload_date:null,link:null})
            setTitle({formTitle:"Add New Document",buttonTitle:"Save"})
        }
    }, [props.open])
    ...
    function checkValidation2() {
        ...
        if(data.document_name == "" || data.document_name == null) {
            isValid = false
            eName = "Document Name can not be empty"
        }
    ...
    const closeForm = () => {
        ...
        listFolder?.map((row,key) => {
            purgeFolderApi(row.id).then((res) => {
                ...
                <Box className="modal-content">
                    ...
                    <Box className="mb-3">
                        {/*<FileUploadComponent3 
                            id="link"
                            path={folder}
                            fileUploaded={(resp) => onUploadedFile(resp)}
                            url={data.link}
                            deleteFile={() => onUploadedFile({link:null, content_type:null})}/>*/}
                        <DropzoneArea
                    ...
                    <Box className="mb-3">
                        ...
                        {listData?.map((row, key) => (
                            <li key={key}>
                                <Chip label={row.document_name} onDelete={() => deleteFileProcess(row.link,key)} className={classes.chip} />
                            </li>
                        ))}
                        </Paper>
                    </Box>
                    <Box className="mb-3">
                        {listProgress?.map((item,key) => (
                            <Box className={classes.progress}>
                                ...
                                <Box className={classes.pbar} alignItems="center">
                                    <Box width="100%" mr={1}>
