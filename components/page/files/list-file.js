...
import { getDetailFolderUserApi,getListFolderUserApi,deleteFolderUserApi } from "../../../services/api/folder-user.api"
import { getListFileUserApi,deleteFileUserApi,downloadFileUserApi,getDetailFileUserApi } from "../../../services/api/file-user.api"
...
export default function Page() {
    ...
    const deleteFolder = async () => {
        ...
        try {
            setOpen(true)
            var res = await deleteFolderUserApi(idFolder)
            var data = await getListFolderUserApi("",0,999,folderId)
    ...
    const deleteDoc = async () => {
        ...
        try {
            setOpen(true)
            var res = await deleteFileUserApi(idData)
            var data = await getListFileUserApi(folderId)
            var dialState = [...openDial]
            var hiddenState = [...hidden]
    ...
    useEffect(async () => {
        try {
            ...
            var vw = switchView()
            setView(vw)
            var fld = await getDetailFolderUserApi(folderId)
            setFolder(fld)
            ...
            setTree(section)
            var flist = await getListFolderUserApi("",0,999,folderId)
    ...
    const handleChangePage = async (event, newPage) => {
        try {
            setOpen(true)
            var data = await getListFileUserApi(folderId,search,newPage,rowsPerPage)
            var dialState = [...openDial]
            var hiddenState = [...hidden]
            var pos = listFolder?.length ?? 0
            for (var i=0; i<data.total; i++) {
    ...
    const handleChangeRowsPerPage = async (event) => {
        ...
        try {
            setOpen(true)
            var data = await getListFileUserApi(folderId,search,0,parseInt(event.target.value, 10))
            var dialState = [...openDial]
            var hiddenState = [...hidden]
            var pos = listFolder?.length ?? 0
            for (var i=0; i<data.total; i++) {
    ...
    const doSearch = async () => {
        try {
            setOpen(true)
            var data = await getListFileUserApi(folderId,search,0,rowsPerPage)
            var dialState = [...openDial]
            var hiddenState = [...hidden]
            var pos = listFolder?.length ?? 0
            for (var i=0; i<data.total; i++) {
    ...
    const refreshListDoc = async () => {
        ...
        try {
            setOpen(true)
            var data1 = await getListFolderUserApi("",0,999,folderId)
            setListFolder(data1.result)
            //setRowCount(data1.total)
            var data2 = await getListFileUserApi(folderId,search,page,rowsPerPage)
    ...
    const newFolder = () => {
        ...
        setDataFolder({id:null,folder_name:null,parent_id:folderId,parent_tree:newTree,teams:folder.teams,users:folder.users,locked:folder.locked,created_by:folder.created_by})
    ...
    const editForm = async (id) => {
        setOpen(true)
        var result = await getDetailFileUserApi(id)
        setData({folder:"",id:id,document_name:result.document_name,folder_id:folderId,upload_date:result.upload_date,link:result.link})
        setOpen(false)
        setOpenForm(true)
    }
    ...
    const isLocked = () => {
        if (userId != folder.created_by && folder.locked) {
            return true
        } else {
            return false
        }
    ...
    return <BaseLayout title="My Files">
        <Box className="p-5 content-wrapper">
                        ...
                        <Box className="card no-padding">
                            ...
                            <TableContainer component={Card}>
                                <Table aria-label="simple table">
                                        ...
                                        <TableRow>
                                            <TableCell>
                                                <Tooltip title="Up to parent folder" placement="top">
                                                    <Link href={getRoute("files")+"/"+folder.parent_id}><Icon>drive_folder_upload</Icon></Link>
                                                </Tooltip>
                                            </TableCell>
                                            <TableCell colspan={5}>
                                                <Tooltip title="Up to parent folder" placement="top">
                                                    <Link href={getRoute("files")+"/"+folder.parent_id}>..</Link>
                                                </Tooltip>
                                            </TableCell>
                                        </TableRow>
                                    ...
                                    {listData?.length == 0 && 
                                        <TableRow>
                                            <TableCell colspan={6} className="text-center text-muted" align="center">No file to show</TableCell>
                                        </TableRow>
                                    }
                                    </TableBody>
                                    <TableFooter>
                                ...
                                {listFolder?.map((row, key) => (
                                    ...
                                    >
                                        <Box className="card-small" onMouseEnter={() => handleHidden(key,false)} onMouseLeave={() => handleHidden(key,true)}>
                                            <Link href={getRoute("files")+"/"+row.id} onClick={(e) => openPage(e, "/files/"+row.id)}>
            ...
            <FileForm open={openForm} closeModal={refreshListDoc} doc={data} />
