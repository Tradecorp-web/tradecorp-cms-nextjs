...
import { getListOrderIdQuotxeApi } from "../../../services/api/order-quote.api";
  ...
  return (
    <React.Fragment>
      <TableRow key={props.keys} className={classes.root}>
        ...
        <TableCell style={{ textAlign: "center" }}>
          <React.Fragment>
            <Tooltip title="Expand container details" placement="top">
              <IconButton
        ...
        <TableRow>
          <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={10}>
            <Collapse in={open} timeout="auto" unmountOnExit>
              <Box style={{ paddingBottom: 5, paddingLeft: 50, paddingTop: 5 }}>
                <Grid container>
                  <Grid item xs="6">
                ...
                <Table size="small" aria-label="materials">
                  ...
                  <TableBody>
                    {props?.row?.container_details?.map((item, key) =>
                      item?.container_id != null ? (
                        <TableRow key={key}>
                          <TableCell>
                            {item?.container_type_data?.name}
                            {item?.container_type_data?.alias != ""
                              ? " (" + item?.container_type_data?.alias + ") "
                              : ""}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            {/* <Button
                              ...
                              variant="outlined"
                              onClick={() => {
                                newContainerOrders(props?.row?.id, item);
                              }}
                            >
                              Container Serial
                            </Button> */}
...
export default function Page() {
  ...
  const handleChangePage = async (event, newPage) => {
    try {
      setOpen(true);
      var data = await getListQuoteInApi(search, newPage, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setPage(newPage);
      setOpen(false);
  ...
  const doSearch = async () => {
    try {
      setOpen(true);
      var data = await getListQuoteInApi(search, 0, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setPage(0);
      setOpen(false);
  ...
  const buildForm = (e, id) => {
    openPage(e, getRoute("quotein-build-quote", { id: id }));
  };

  const exportForm = async (e, id) => {
    // var dataOrder = await getListOrderIdQuoteApi(id, "*");
    ...
    var result = await getDetailQuoteInApi(id).then((result) => {
      if (result.id != null) {
        setData({
          ...
          quote_in_number: result.quote_in_number,
  ...
  const changeSort = async (field) => {
    ...
    try {
      setOpen(true);
      var data = await getListQuoteInApi(search, page, rowsPerPage, field, ord);
      setRowCount(data.total);
      setListData(data.result);
      setOpen(false);
    } catch (err) {
      ...
      <Box className="p-5 content-wrapper">
        ...
        >
          <Grid item xs={12} lg={12} xl={12}>
            <h1 className="mb-3">Build Quote</h1>
                  ...
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell
                        ...
                        key="quote_in_number"
                        sortDirection={
                          orderBy === "quote_in_number" ? order : false
                        }
                      >
                        <TableSortLabel
                          active={orderBy === "quote_in_number"}
                          direction={
                            orderBy === "quote_in_number" ? order : "asc"
                          }
                          onClick={() => changeSort("quote_in_number")}
                        >
                          Quote Number
                        </TableSortLabel>
                      </TableCell>
