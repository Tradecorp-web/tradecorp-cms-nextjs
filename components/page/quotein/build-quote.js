...
import SendToProduction from "./form-send-production";
...
export default function BuildQuoteInput() {
  ...
  useEffect(async () => {
    ...
    if (data?.quote_status == "production") {
      setMsgStatusQuote("Already Sent to Production");
    }

    setQuoteData(data);
  }, []);
  useEffect(async () => {
    var data = await getListOrderIdQuoteApi(quoteinId, "*");
  ...
  const handleClickOpenProduction = () => {
    updateStatusOrderQuoteApi(quoteinId, "production");
    setMsgStatusQuote("Already Sent to Production");
  ...
  return (
    <BaseLayout title="Quotation">
                      ...
                      >
                        ...
                        <TableBody>
                          {orderDataList?.result?.map((res, key) => (
                            <TableRow key={key}>
                              <TableCell>{res?.unit_code}</TableCell>
                              <TableCell>
                                {res?.color}{" "}
                              ...
                              <TableCell>{res?.container_number_to}</TableCell>
                              <TableCell>{res?.customer?.company}</TableCell>
                              <TableCell>
                                <IconButton>
                                  <Edit onClick={() => clickEditRow(res?.id)} />
                                </IconButton>
                              </TableCell>
                            </TableRow>
                          ))}
      ...
      <UploadForm
        ...
        uploadData={fUploadData}
        uploadListData={uploadListData}
        uploadOtherListData={uploadOtherListData}
        uploadOtherData={fUploadOtherData}
        keyRow={keyRow}
      ></UploadForm>
      <SpecificationForm
        open={openSpecificationForm}
        closeModal={fSpecData}
