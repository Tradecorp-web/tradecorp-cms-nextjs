...
export default function Page() {
  ...
  useEffect(async () => {
    try {
      setOpen(true);
      var data = await getSOProjectIncomeApi(projectCode, "*");
  ...
  const refreshListSO = async () => {
    ...
    try {
      setOpen(true);
      var data = await getSOStatusAccApi(-1, search, page, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setOpen(false);
    } catch (err) {
  ...
  return (
    <BaseLayout title="Edit Detail">
      <div className={classes.root}>
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <Typography variant="subtitle1">Edit Detail Sales Order</Typography>
          </Grid>
        </Grid>
        <Grid container spacing={2}>
          <Grid item xs={12}>
