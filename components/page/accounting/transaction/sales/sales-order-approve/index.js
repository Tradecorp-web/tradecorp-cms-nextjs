    ...
    <React.Fragment>
      <TableRow key={props.keys} hover className={classes.root}>
        ...
        <TableCell align={"center"}>
          ...
          {props.row.payment_approve === null && (
            <span
              className={classes.descriptionbase}
...
export default function Page() {
  ...
  const deleteSO = async () => {
    ...
    try {
      ...
      var data = await getListSalesOrderApi();
  ...
  const handleChangePage = async (event, newPage) => {
    try {
      ...
      var data = await getListSalesOrderApi(search, newPage, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setPage(newPage);
      setOpen(false);
  ...
  const doSearch = async () => {
    try {
      setOpen(true);
      var data = await getListSalesOrderApi(search, 0, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setPage(0);
      setOpen(false);
  ...
  const refreshListSO = async () => {
    ...
    try {
      setOpen(true);
      var data = await getListSalesOrderApi(search, page, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setOpen(false);
    } catch (err) {
  ...
  const editForm = async (id) => {
    router.push("/accounting/transaction/sales/sales-order-approve/" + id);
  };
