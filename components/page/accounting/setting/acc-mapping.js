...
import {
  getListAccMappingApi,
  getDetailAccMappingApi,
} from "../../../../services/api/acc-mapping.api";
  ...
  return (
    <React.Fragment>
      <TableRow key={props.keys} hover className={classes.root}>
        ...
        <TableCell onClick={() => props.clickAction()}>
          {props.row.application_code}
        </TableCell>
        <TableCell onClick={() => props.clickAction()}>
          {props.row.application_name}
        </TableCell>
        <TableCell onClick={() => props.clickAction()}>
          {props.row.accounting_coa_code}
        </TableCell>
        <TableCell onClick={() => props.clickAction()}>
          <FormControl component="fieldset">
            ...
            >
              <FormControlLabel
                ...
                label="Debit"
                labelPlacement="debit"
                checked={props.row.accounting_debit ? "checked" : ""}
              />
              <FormControlLabel
                value="credit"
                control={<Radio color="primary" />}
                label="Credit"
                labelPlacement="credit"
                checked={props.row.accounting_credit ? "checked" : ""}
              />
            </RadioGroup>
          </FormControl>
        </TableCell>
...
export default function Page() {
  ...
  useEffect(async () => {
    try {
      setOpen(true);
      var data = await getListAccMappingApi();
  ...
  const handleChangePage = async (event, newPage) => {
    try {
      setOpen(true);
      var data = await getListAccMappingApi(search, newPage, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setPage(newPage);
      setOpen(false);
  ...
  const doSearch = async () => {
    try {
      setOpen(true);
      var data = await getListAccMappingApi(search, 0, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setPage(0);
      setOpen(false);
  ...
  const refreshListCoa = async () => {
    ...
    try {
      setOpen(true);
      var data = await getListAccMappingApi(search, page, rowsPerPage);
      setRowCount(data.total);
      setListData(data.result);
      setOpen(false);
    } catch (err) {
  ...
  return (
    <AccountingBaseLayout title="Application COA Mapping">
      ...
      <Box className="p-5 content-wrapper">
        ...
        >
          <Grid item xs={12} lg={12} xl={12}>
            <h1 className="mb-3">Application Mapping</h1>
