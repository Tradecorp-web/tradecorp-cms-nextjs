...
export default function Page() {
    ...
    async function deleteData() {
        ...
        await deleteVehicleCategoryApi(data?.id)
        setEditData(null)
    }
    // --------< Delete Data >--------
    ...
    var listVehicleCategorySwr = getListVehicleCategorySwr({
        search: search, 
        page: page+1, 
        limit: limit,
        orderBy: "name",
    ...
    useEffect(() => {
        setLoading(listVehicleCategorySwr?.isLoading)
        if(listVehicleCategorySwr?.data?.result) {
            setTotal(listVehicleCategorySwr?.data?.total)
            setDataList(listVehicleCategorySwr?.data?.result)
        }
    }, [listVehicleCategorySwr])
    // ------------------------------------------
    ...
    return <BaseLayoutVehicle title="Vehicle Category">
        <div className="p-5 content-wrapper">
            <Grid container className="page-container" alignItems="center" justify="center">
                <Grid item xs={12} lg={8}>
                    <h1 className="mb-3">Vehicle Category</h1>
                    <div className="card no-padding">
                        <div className="p-3 display-space-between">
                            <div className="display-space-between" style={{width: "25%"}}>
                                <IconButton onClick={() => router.push(getRoute("vehicle"))} size="small" className="me-3"><Icon>arrow_back</Icon></IconButton>
        ...
        <VehicleCategoryForm 
            ...
            isEdit={isFormEdit}
            data={editData}
            dataInserted={(data) => listVehicleCategorySwr.mutate()}
            dataUpdated={(data) => listVehicleCategorySwr.mutate()} />
        <AlertDialog 
            title="Delete Item"
