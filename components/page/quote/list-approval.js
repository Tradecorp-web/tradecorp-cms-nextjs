import { Button, ButtonGroup, Card, makeStyles, Divider, Grid, Icon, IconButton, Table, TableBody, TableCell, TableContainer, TableFooter, TableHead, TablePagination, TableRow, Typography, Box, TextField, Backdrop, CircularProgress, Collapse, Tooltip } from "@material-ui/core"
import { useRouter } from "next/router"
import React, { useEffect, useState } from "react"
import getRoute from "../../../helpers/router"
import { PlaylistAddCheck,KeyboardArrowUp,KeyboardArrowDown } from '@material-ui/icons'
import { getListQuoteApi,getDetailQuoteApi } from "../../../services/api/quote.api"
    ...
    return (
      <React.Fragment>
        <TableRow key={props.keys} className={classes.root}>
          ...
          <TableCell>{props.row.customer.company}</TableCell>
          ...
          <TableCell>
            <Tooltip title="Approval" placement="top">
                <IconButton>
                    <PlaylistAddCheck onClick={() => props.clickAction()} />
                </IconButton>
            </Tooltip>
          </TableCell>
        </TableRow>
...
export default function Page() {
    ...
    const editForm = async (id) => {
        ...
        setData({id:id,name:result.name,quote_date:result.quote_date,sales_id:result.sales_id,customer_id:result.customer_id,containers:result.containers,status:result.status,products:result.products})
        setOpen(false)
        setOpenForm(true)
    }

    return <BaseLayout title="Quotation Approval">
        <Box className="p-5 content-wrapper">
            <Grid container className="page-container" alignItems="center" justify="center">
                <Grid item xs={12} lg={12} xl={8}>
                        <h1 className="mb-3">Quotation Approval</h1>
                            ...
                            <TableContainer component={Card}>
                                <Table aria-label="simple table">
                                    ...
                                    {listData?.map((row, key) => (
                                        <Row keys={key} row={row} clickAction={() => editForm(row.id)} page={page} rowsPerPage={rowsPerPage} />
                                    ))}
                                    {listData?.length == 0 && 
                                        <TableRow>
