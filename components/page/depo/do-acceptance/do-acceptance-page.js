...
import DOAcceptanceForm from './do-acceptance-form';
...
export default function DataTable() {
  ...
  var listDoAcceptanceSwr = getListDOAcceptanceSwr({
      search: search, 
      page: page+1, 
      limit: limit,
      orderBy: "code",
  ...
  useEffect(() => {
    setLoading(listDoAcceptanceSwr?.isLoading)
    if(listDoAcceptanceSwr?.data?.result) {
      setTotal(listDoAcceptanceSwr?.data?.total)
      setDataList(listDoAcceptanceSwr?.data?.result)
    }
  }, [listDoAcceptanceSwr])
  ...
  return (
    <BaseLayoutDepo title="DO Acceptance">
      <AlertDialog 
            title="Delete Item"
      ...
      <div className="p-5 content-wrapper">
        <Grid container className="page-container" alignItems="center" justify="center">
          <Grid item xs={12} lg={12} xl={10}>
            <h1 className="mb-3">DO Acceptance</h1>
                ...
                <div className="p-3 display-space-between">
                  ...
                  <ButtonGroup variant="outlined" color="default" ref={anchorRef} aria-label="split button">
                      <Button
                          ...
                          <Icon>add</Icon>Create Do Acceptance
                      </Button>
                      {/* <Button
                          size="small"
                        ...
                        <TableBody>
                          {!isLoading && dataList.map((data, index) => (
                              <TableRow key={index} hover={true}>
                                  <TableCell onClick={(e) => openDetail(e, getRoute('depo.do.accept.detail', {depoSlug: depoId, id: data?.id}))}>{index + 1 + (page*limit)}</TableCell>
                                  <TableCell onClick={(e) => openDetail(e, getRoute('depo.do.accept.detail', {depoSlug: depoId, id: data?.id}))}>{data?.reference_number}</TableCell>
                                  <TableCell onClick={(e) => openDetail(e, getRoute('depo.do.accept.detail', {depoSlug: depoId, id: data?.id}))}>{dateFormat(data?.date)}</TableCell>
                                  <TableCell onClick={(e) => openDetail(e, getRoute('depo.do.accept.detail', {depoSlug: depoId, id: data?.id}))}>{data?.stocks?.length}x GP</TableCell>
                                  <TableCell onClick={(e) => openDetail(e, getRoute('depo.do.accept.detail', {depoSlug: depoId, id: data?.id}))}>{data?.customer?.name} ({data?.customer?.company})</TableCell>
                                  <TableCell onClick={(e) => openDetail(e, getRoute('depo.do.accept.detail', {depoSlug: depoId, id: data?.id}))}>{data?.creator?.name}</TableCell>
                                  <TableCell onClick={(e) => openDetail(e, getRoute('depo.do.accept.detail', {depoSlug: depoId, id: data?.id}))}>{data?.remarks}</TableCell>
                                  {/* <TableCell>
      ...
      <DOAcceptanceForm 
          ...
          dataInserted={(data) => listDoAcceptanceSwr.mutate()} />
