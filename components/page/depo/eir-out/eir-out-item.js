...
export default function EirOutItem(props) {
    ...
    function openDetailContainer(e) {
        if(!(props?.active)) {
            handleMenuClose()
            e.preventDefault()
            router.push(getRoute("depo.eir.out.detail", {depoSlug: depoSlug, eirNo: props?.data?.id}))
        }
    }
    ...
    return (
        <div className={"stock-container-item " + (props?.active ? "active" : "")}>
            <Link 
                href={!(props?.active) ? getRoute("depo.eir.out.detail", {depoSlug: depoSlug, eirNo: props?.data?.id}) : "#"} 
