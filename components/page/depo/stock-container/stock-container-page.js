...
import { getListContainerStockSwr, getListContainerStock2Swr, getListContainerStock3Swr } from '../../../../services/swr/container-stock.swr';
import { getCountryListSwr } from '../../../../services/swr/countries-cities.swr'
import { getColorListSwr } from '../../../../services/swr/color-codes.swr';
...
import { getDetailDepoSwr, getListDepoSwr, getListDepoNoLimitSwr, getListDepoGSwr } from "../../../../services/swr/depo.swr";
import StockContainerForm from '../../container/container-form'
import StockContainerImport from '../../container/container-import';
import ContainerImportHistory from '../../container/container-import-history';
...
import { LOCAL_STORAGE_STOP_TIMEOUT, LOCAL_STORAGE_DEPO_GROUP } from "../../../../helpers/consts";
import { exportContainerStock } from "../../../../services/export/export-container"
...
export default function StockContainerPage() {
  ...
  const [groupDepo, setGroupDepo] = useState("")
  const [mColorOptions, setMColorOptions] = useState([])
  const depoSwr = getDetailDepoSwr(depoSlug)
  useEffect(() => {
  ...
  var colorListSwr = getColorListSwr("asc", "name_english")

  useEffect(() => {
    setMColor(colorListSwr?.data);
    if (colorListSwr?.data) {
      setMColor(colorListSwr?.data);
    }
  }, [colorListSwr])
  ...
  const [param, setParam] = useState({ page: (page + 1).toString(), limit: limit.toString(), orderBy: "serial_number", order: "asc", serial_number: null, size_id: null, type_id: null, repair_status_id: null, condition_id: null, percentage: null, yom_year: null, gate_in_date: null, depo_id: JSON.stringify([depoSlug]), stock_status_id: null, delivery_status_id: null, selling_price: null, owner: null, stock_reconcile: null, remarks: null, one_way_ref: null, supplier_release: null, supplier_invoice: null, depo_release: null, ral_code: null, name_english: null })
  const [choice, setChoice] = useState({ size_id: [], type_id: [], repair_status_id: [], condition_id: [], depo_id: [], stock_status_id: [], stock_reconcile: [], delivery_status_id: [], name_english: [] })
  const [anchor, setAnchor] = useState({ serial_number: null, size_id: null, type_id: null, repair_status_id: null, condition_id: null, percentage: null, yom_year: null, gate_in_date: null, depo_id: null, delivery_status_id: null, stock_status_id: null, selling_price: null, owner: null, stock_reconcile: null, remarks: null })
  ...
  var headCells = [
    ...
    { id: "delivery_status_id", label: "Delivery Status", sort: true, filter: true, filter_type: "select" },
    { id: "stock_status_id", label: "Sale Status", sort: true, filter: true, filter_type: "select" },
  ]

  if (sellingPrice) {
    headCells.push({ id: "selling_price", label: "Selling Price", sort: true, filter: true, filter_type: "numeric" })
  }
  headCells.push({ id: "owner", label: "Owner", sort: true, filter: true, filter_type: "string" })
  headCells.push({ id: "stock_reconcile", label: "Stock Reconcile", sort: true, filter: true, filter_type: "select" })
  headCells.push({ id: "one_way_ref", label: "One Way Reference", sort: true, filter: true, filter_type: "string" })
  headCells.push({ id: "supplier_release", label: "Supplier Release", sort: true, filter: true, filter_type: "string" })
  headCells.push({ id: "supplier_invoice", label: "Supplier Invoice", sort: true, filter: true, filter_type: "string" })
  headCells.push({ id: "depo_release", label: "Depo Release", sort: true, filter: true, filter_type: "string" })
  headCells.push({ id: "name_english", label: "Color", sort: true, filter: true, filter_type: "select" })
  headCells.push({ id: "remarks", label: "Remark", sort: true, filter: true, filter_type: "string" })

  const columnCount = headCells.length + 3

  var listContainerStockSwr = getListContainerStock3Swr(param)
  ...
  const [depoNoLimitList, setDepoNoLimitList] = useState([])
  ...
  var depoSwrList = getListDepoGSwr(
    {
      limit: 999,
      orderBy: "name",
      order: "asc",
  ...
  // var depoNoLimitListSwr = getListDepoNoLimitSwr({ group: localStorage.getItem(LOCAL_STORAGE_DEPO_GROUP) });
  var depoNoLimitListSwr = getListDepoNoLimitSwr({ group: "-" });
  ...
  useEffect(() => {
    if (depoNoLimitListSwr?.data?.result) {
      setDepoNoLimitList(depoNoLimitListSwr?.data?.result ?? [])
    }
  }, depoNoLimitListSwr.data)

  useEffect(() => {
    if (depoList != null && masterData != null) {
      getContainerOptionApi(depoSlug)
        .then(res => {
          ...
          if (res.delivery_status_option.length > 0) {
            setDeliveryOptions(masterData.filter(val => val?.category == "delivery" && res.delivery_status_option.indexOf(val?.id) !== -1) ?? [])
          }
          ...
          if (res.color_name_option.length > 0) {
            setMColorOptions(mColor.filter(val => res.color_name_option.indexOf(val?.name_english) !== -1) ?? [])
          }
  ...
  const changeFilter = (e) => {
    if (Array.isArray(e.target.value)) {
      ...
      } else if (e.target.name == "name_english") {
        setParam({ ...param, color_name: JSON.stringify(e.target.value) })
      } else {
  ...
  const selectedChoice = (name, selected) => {
    ...
    } else if (name == "delivery_status_id") {
    ...
    } else if (name == "name_english") {
      mColorOptions.map((item, i) => {
        if (selected.includes(item.name_english)) {
          out.push(item.name_english)
        }
      })
    }
    return out.join(",")
  ...
  const clearFilter = () => {
    setChoice({ size_id: [], type_id: [], repair_status_id: [], condition_id: [], depo_id: [], stock_status_id: [], stock_reconcile: [], delivery_status_id: [], name_english: [] })
    setParam({ page: (page + 1).toString(), limit: limit.toString(), orderBy: "serial_number", order: "asc", serial_number: null, size_id: null, type_id: null, repair_status_id: null, condition_id: null, percentage: null, yom_year: null, gate_in_date: null, depo_id: JSON.stringify([depoSlug]), stock_status_id: null, delivery_status_id: null, selling_price: null, supplier_release: null, supplier_invoice: null, owner: null, stock_reconcile: null, remarks: null, name_english: null })
  }
  // ------------------------------------------
  ...
  const handleMenuItemClick = async (e, index) => {
    e.preventDefault()
    var menu = options[index]
    if (menu?.value == 1) {
      var params = { ...param }
      params.page = "1"
      params.limit = total.toString()
      await exportContainerStock(params)
    }
    if (menu?.value == 2) {
      setOpenImport(true)
    }
  ...
  const options = [
    { name: "Export to Excel", value: 1 },
    { name: "Import Container Stock", value: 2 },
  ]
  if (isAdmin) {
    options.push({ name: "Import History", value: 3 })
  }
  ...
  const refreshMaster = (update) => {
    masterSwr.mutate()
    if (lastForm == "import") {
      setNewOptionImport(update)
      setOpenImport(true)
    } else if (lastForm == "insert") {
  ...
  return (
    <BaseLayoutDepo title="Container Stock">
      <AlertDialog
        title="Delete Item"
                ...
                <Box className="p-3 display-space-between">
                  ...
                  <ButtonGroup variant="outlined" color="default" ref={anchorRef} aria-label="split button">
                    <Button
                      ...
                      <Icon>add</Icon>Add Container Stocks
                    </Button>
                    <Button
                      size="small"
              ...
              <TableContainer>
                <Table aria-label="simple table">
                              ...
                              <Box style={{ whiteSpace: "nowrap" }}>
                                ...
                                >
                                  <Paper className="p-1">
                                    ...
                                    {(headCell.filter && headCell.id == "delivery_status_id") &&
                                      <FormControl className={classes.formControl}>
                                        <InputLabel id="delivery-status-label" className={classes.selectLabel}>{headCell.label}</InputLabel>
                                        <Select
                                    ...
                                    {(headCell.filter && headCell.id == "name_english") &&
                                      <FormControl className={classes.formControl}>
                                        <InputLabel id="depo-label" className={classes.selectLabel}>{headCell.label}</InputLabel>
                                        <Select
                                          labelId="depo-label"
                                          ...
                                          onChange={changeFilter}
                                        >
                                          {mColorOptions?.map((row, key) => {
                                            return (
                                              <MenuItem key={key} value={row.name_english}>
                                                <Checkbox checked={choice[headCell.id].indexOf(row.name_english) > -1} />
                                                <ListItemText primary={row.name_english} />
                                              </MenuItem>
                                            )
                                          })}
                                        </Select>
                  ...
                  <TableBody>
                    {!isLoading && dataList.map((data, index) => (
                      <TableRow key={index} hover={true}>
                        ...
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{index + 1 + (page * limit)}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.serial_number}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.size?.name}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.type?.name}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.repair_status?.name}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.condition?.name}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.percentage ?? 0}%</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.yom_month}-{data?.yom_year}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.gate_in_date ? Moment(data?.gate_in_date).format("LL") : "-"}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>
                          {(data?.stock_status_id != 1002 && data?.stock_status_id != 1003) &&
                            countDays(data?.gate_in_date)
                          }
                        </TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.depo?.name}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.delivery_status?.name}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.stock_status?.name}</TableCell>
                        {sellingPrice &&
                          <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{currency(data?.selling_price, data?.selling_currency + " ")}</TableCell>}
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.owner}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))} align="center">{data?.stock_reconcile && <Done style={{ color: "green" }} />}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.one_way_ref}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.supplier_release}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.supplier_invoice}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.depo_release}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))} align="center">{data?.color_name}{data?.color_code != undefined ? data?.color_code != "" ? mColor && (<Icon fontSize="small" style={{ color: mColor?.find(item => item?.ral_code == data?.color_code)?.html_code }}  >brightness_1</Icon>) : "" : ""}</TableCell>
                        <TableCell onClick={(e) => openDetail(e, getRoute('depo.stock.detail', { depoSlug: depoSlug, id: data?.id }))}>{data?.remarks}</TableCell>
                        <TableCell>
                          {isAdmin &&
                  ...
                  <TableFooter>
                    <TableRow>
                      <TablePagination
                        rowsPerPageOptions={[10, 20, 50, 100, 1000, 2000, 5000]}
                        colSpan={columnCount}
                        count={total}
                        rowsPerPage={limit}
                        page={page}
      ...
      <StockContainerImport
        open={openImport}
        closeModal={() => setOpenImport(false)}
        stockDepoId={depoSlug}
        dataImported={(id) => refreshData(id)}
        addMaster={(category) => addMaster(category, "import")}
        newOption={newOptionImport} />
      <ContainerImportHistory
        open={openHistory}
        closeModal={() => setOpenHistory(false)}
        dataRefresh={(id) => refreshData(id)} />
