import { Divider, Typography, Grid, Paper, IconButton, Icon, Button, Link, Box, Chip, Collapse, Table, TableHead, TableBody, TableFooter, TableRow, TableCell, TableContainer, Snackbar, Tabs, Tab, Modal, Card, CardMedia, CardActions, makeStyles, CircularProgress, Tooltip, CardContent, ButtonGroup, Popper, Grow, ClickAwayListener, MenuList, MenuItem, Checkbox, LinearProgress } from "@material-ui/core";
...
import StockContainerLayout from "./stock-container-layout";
import { isPermit, currency, countDays, switchView } from "../../../../helpers/general";
...
import StockContainerAttachmentForm from "../../container/container-attachment-form";
import StockContainerAddAttachmentForm from "../../container/container-attachment-add-form";
import StockContainerMoveAttachmentForm from "../../container/container-attachment-move-form";
...
export default function StockContainerDetail() {
    ...
    const containerSwr = getDetailContainerStockSwr(serialNumber)
    
    useEffect(() => {
        if (containerSwr.data != null) {
            if (containerSwr.data.stock_reconcile) {
                setTitleBackground("-webkit-linear-gradient(left,#fafafa 0px,#196f3d 50px,#fafafa,#fafafa)")
            } else {
                setTitleBackground("")
            }
            var cntr = containerSwr.data
            var history = []
            for (var i=cntr.image_history.length-1; i>=0; i--) {
                history.push(cntr.image_history[i])
            }
            cntr.image_history = history
            setContainer(cntr)
        }
    }, [containerSwr.data])
    ...
    useEffect(() => {
        if (container != null) {
            ...
            if (container.image_history != null) {
                for (var i=0; i<container.image_history.length; i++) {
                    dialState.history[i] = false
                    hiddenState.history[i] = true
                }
            }
            ...
            preLoadFile("history", container.image_history)
        }
    }, [container])
    ...
    const nextFile = (content) => {
        ...
        if (content == "image") {
            ...
            if (newKey == container.image_history.length) {
                newKey = 0
            }
            downloadFile(content,newKey,container.image_history[newKey].link,container.image_history[newKey].content_type,container.image_history[newKey].label)
        }
    }
    ...
    const prevFile = (content) => {
        ...
        if (content == "image") {
            ...
            if (newKey < 0) {
                newKey = container.image_history.length - 1
            }
            downloadFile(content,newKey,container.image_history[newKey].link,container.image_history[newKey].content_type,container.image_history[newKey].label)
        }
    }
    ...
    //             setSelView({tipe: "attach", key:name, name:name, blob:blobAttach[name]})
    //             setOpenView(true)
    //         } else {
    ...
    //                     setSelView({tipe: "attach", key:name, name:name, blob:bl[name]})
    //                     setOpenView(true)
    //                 } else {
    ...
    const [loadProgress, setLoadProgress] = useState({all:false,image:false,csc:false,specification:false,contract:false,invoice:false,drawing:false,survey:false,other:false,original:false,history:false})
    const [loadBar, setLoadBar] = useState({all:0,image:0,csc:0,specification:0,contract:0,invoice:0,drawing:0,survey:0,other:0,original:0,history:0})
    ...
    const downloadAll = async (cat) => {
        try {
            ...
            } else if (cat == "image_history") {
                if (progress.history) {
                    return
                }
                progress.history = true
            }
            ...
            if (response.status == 200) {
                ...
                while (true) {
                    ...
                    if (cat == "all") {
                        ...
                        bar.history = Math.round(parseInt(receiveLen / len * 100))
                    }
                    setLoadBar({...bar})
                }
                if (cat == "all") {
    ...
    const [hidden, setHidden] = useState({image:[],csc:[],specification:[],contract:[],invoice:[],drawing:[],survey:[],other:[],original:[],history:[]})
    const [openDial, setOpenDial] = useState({image:[],csc:[],specification:[],contract:[],invoice:[],drawing:[],survey:[],other:[],original:[],history:[]})
    ...
    const closeEditLabel = () => {
        ...
        } else if (record.field == "image_history") {
            handleOpenDial("history",record.key,false)
            handleHidden("history",record.key,true)
        }
    }
    ...
    const [movePick, setMovePick] = useState({image:false,csc:false,specification:false,contract:false,invoice:false,drawing:false,survey:false,other:false,original:false,history:false})
    const [listKey, setListKey] = useState({image:[],csc:[],specification:[],contract:[],invoice:[],drawing:[],survey:[],other:[],original:[],history:[]})
    const [listCheck, setListCheck] = useState({image:[],csc:[],specification:[],contract:[],invoice:[],drawing:[],survey:[],other:[],original:[],history:[]})
    ...
    const moveAttachment = (tipe) => {
        if (container[tipe] != null) {
            if (container[tipe].length > 0) {
                ...
                } else if (tipe == "image_history") {
                    setList(listKey.history)
                }
                setOrigin(tipe)
                setOpenMove(true)
            } else {
    ...
    const closeMove = () => {
        ...
        } else if (origin == "image_history") {
            var check = {...listCheck}
            for (var i=0; i<check.history.length; i++) {
                check.history[i] = false
            }
            setListKey({...listKey, history:[]})
            setListCheck(check)
            setMovePick({...movePick, history:false})
        }
        setList([])
        setOrigin("")
        setOpenMove(false)
        ...
        <StockContainerLayout serialNumber={container?.serial_number} title={`Stock: ${container?.serial_number}`}>
            <Box className="display-space-between mb-3" style={{backgroundImage:titleBackground}}>
                <Box className="mb-3" style={{marginTop:15}}>
                    ...
                    <Box className={classes.title} style={{color:"darkgreen"}}>{container?.serial_number.substring(10)}</Box>}
                    {container?.check_digit != true &&
            ...
            <Tabs value={tabValue} onChange={handleTabChange} aria-label="simple tabs">
                ...
                <Tab label="Image History" id="simple-tab-7" aria-controls="simple-tabpanel-7" />
            </Tabs>
                                ...
                                <Box className="mb-3">
                                    ...
                                    <Box>
                                        <Typography variant="body1" gutterBottom>
                                            ...
                                            {(container?.stock_status_id == 1005 && container?.available_status instanceof Array)? " ("+container?.available_status.join(",")+")" : ""}
                                            {(container?.stock_status_id == 1001 && container?.hold_end_date != null)? " until "+Moment(container?.hold_end_date).format("LL") : ""}
                                        </Typography>
                                    </Box>
                                </Box>
                                <Box className="mb-3">
                            ...
                            <Grid item xs={12} md={12}>
                                <Box className="display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Replace Image" placement="top">
                                            <IconButton onClick={() => addAttachment("images",true)} size="small">
                                                <Icon>add_photo_alternate</Icon>
                                            </IconButton>
                                        </Tooltip>
                                        <Tooltip title="Add new Image" placement="top">
                                            <IconButton onClick={() => addAttachment("images",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                ...
                                <Box className="mb-3 display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Add new CSC Certificate" placement="top">
                                            <IconButton onClick={() => addAttachment("csc_certificates",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                ...
                                <Box className="mb-3 display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Add new Specification" placement="top">
                                            <IconButton onClick={() => addAttachment("specifications",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                ...
                                <Box className="mb-3 display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Add new Contract" placement="top">
                                            <IconButton onClick={() => addAttachment("contracts",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                ...
                                <Box className="mb-3 display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Add new Invoice" placement="top">
                                            <IconButton onClick={() => addAttachment("invoices",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                ...
                                <Box className="mb-3 display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Add new Drawing" placement="top">
                                            <IconButton onClick={() => addAttachment("drawings",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                ...
                                <Box className="mb-3 display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Add new Survey" placement="top">
                                            <IconButton onClick={() => addAttachment("surveys",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                ...
                                <Box className="mb-3 display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Add new Relative Document" placement="top">
                                            <IconButton onClick={() => addAttachment("relative_documents",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                ...
                                <Box className="mb-3 display-space-between">
                                    <Box display="flex">
                                        ...
                                        <Tooltip title="Add new Original Photo" placement="top">
                                            <IconButton onClick={() => addAttachment("original_photos",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
            ...
            <Box role="tabpanel" hidden={tabValue !== 7} id="simple-tabpanel-7" aria-labelledby="simple-tab-7">
                <Box variant="outlined" className="p-4 mb-5 card">
                    <Box className="container-detail-wrapper mb-3">
                        <Grid container className="page-container" spacing={5}>
                            <Grid item xs={12} md={12}>
                                <Box className="display-space-between">
                                    <Box display="flex">
                                        <Typography variant="subtitle1" color="textSecondary">Container Image History</Typography>
                                        {/* <Tooltip title="Add new Image History" placement="top">
                                            <IconButton onClick={() => addAttachment("history",false)} size="small">
                                                <Icon>add</Icon>
                                            </IconButton>
                                        </Tooltip>
                                        <Tooltip title="Move Image History" placement="top">
                                            <IconButton onClick={() => togglePick("history")} aria-label="move" size="small">
                                                <Icon>library_add_check</Icon>
                                            </IconButton>
                                        </Tooltip> */}
                                    </Box>
                                    <Box>
                                        <ButtonGroup variant="contained" color="default" aria-label="split button">
                                            ...
                                            {/* {movePick.image &&
                                            <Button onClick={() => moveAttachment("images")} startIcon={<Send />}>Move selected to</Button>} */}
                                            {(container?.image_history?.length > 0 && !movePick.history) &&
                                            <Button onClick={() => downloadAll("history")} startIcon={!loadProgress.history ? <GetApp /> : ""}>{loadProgress.history ? <React.Fragment><CircularProgress variant="determinate" size={26} value={loadBar.history} />&nbsp;{`${loadBar.history}%`}</React.Fragment> : "Download All Image History"}</Button>}
                                        </ButtonGroup>
                                    </Box>
                                </Box>
                            </Grid>
                            {(view == "list" && container?.image_history?.length > 0) &&
                            <Grid item xs={12} md={12} className={classes.wrapper}>
                                <TableContainer>
                                    <Table size="small" aria-label="simple table">
                                        <TableHead>
                                            <TableRow>
                                                ...
                                                <TableCell width={12}></TableCell>
                                                <TableCell>File Name</TableCell>
                                                <TableCell>Move to History by</TableCell>
                                                <TableCell>At</TableCell>
                                                <TableCell width={12} />
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
                                        {container?.image_history?.map((item, key) => (
                                            <React.Fragment>
                                                {item.label != "" &&
                                                <TableRow>
                                                    <TableCell>
                                                        {movePick.history &&
                                                            <Checkbox
                                                            checked={listCheck.history[key]}
                                                            onClick={(e) => onCheckChange(e,key,"history")}
                                                            color="primary"
                                                        />}
                                                    </TableCell>
                                                    <TableCell>
                                                        {progHistory[key] == 0 &&
                                                        <Tooltip title={`Open ${item.label}`} placement="top">
                                                            <Link onClick={() => downloadFile("history",key,item.link,item.content_type,item.label)}><Icon>insert_drive_file</Icon></Link>
                                                        </Tooltip>}
                                                        {progHistory[key] == 1  &&
                                                        <Icon color="primary">insert_drive_file</Icon>}
                                                    </TableCell>
                                                    <TableCell>
                                                        {progHistory[key] == 0 &&
                                                        <Tooltip title={`Open ${item.label}`} placement="top">
                                                            <Link onClick={() => downloadFile("history",key,item.link,item.content_type,item.label)}>{item.label}</Link>
                                                        </Tooltip>}
                                                        {progHistory[key] == 1 &&
                                                        <Box>
                                                            <Typography color="primary" variant="body2">{item.label}</Typography>
                                                            <Box className={classes.pbar3} alignItems="center">
                                                                <Box width="100%" mr={1}>
                                                                    <LinearProgress variant="determinate" value={barHistory[key]} />
                                                                </Box>
                                                                <Box minWidth={35}>
                                                                    <Typography variant="body2" color="textSecondary">{`${barHistory[key]}%`}</Typography>
                                                                </Box>
                                                            </Box>
                                                        </Box>}
                                                    </TableCell>
                                                    <TableCell>{item.move_user.name}</TableCell>
                                                    <TableCell>{Moment(item.move_at).format("LLL")}</TableCell>
                                                    <TableCell>
                                                        {/* <Tooltip title="Rename" placement="top">
                                                            <IconButton>
                                                                <EditAttributes onClick={() => editLabel("history",key,item.link,item.label)} />
                                                            </IconButton>
                                                        </Tooltip> */}
                                                    </TableCell>
                                                </TableRow>}
                            ...
                            {container?.image_history?.map((item, key) => {
                                return <React.Fragment>
                                    {item.label != "" &&
                                    <Grid item xs={12} md={2} className={classes.wrapper}>
                                        <Box className="card-small" onMouseEnter={() => handleHidden("history",key,false)} onMouseLeave={() => handleHidden("history",key,true)}>
                                            {movePick.history &&
                                            <Box class={classes.checkContainer}>
                                                <Checkbox
                                                    checked={listCheck.history[key]}
                                            ...
                                            {progHistory[key] == 0 &&
                                            <Link onClick={() => downloadFile("history",key,item.link,item.content_type,item.label)}>
                                                <Box class={classes.itemContainer} justifyContent="center" alignItems="center">
                                                    <img src={`data:image/jpeg;base64,${item.thumbnail}`} />
                                                    <Typography variant="h4" component="h4" style={{wordWrap:"anywhere",marginLeft:"0px"}}>{item.label}</Typography>
                                                </Box>
                                            ...
                                            <Box class={classes.itemContainer} justifyContent="center" alignItems="center">
                                                ...
                                                <Box class={classes.pContainer}>
                                                    <Box class={classes.pbar}>
                                                        <CircularProgress variant="determinate" value={barHistory[key]} />
                                                        <Box top={0} left={0} bottom={0} right={0} alignItems="center" className={classes.pbar2}>
                                                            <Typography variant="body2" color="textSecondary">{`${barHistory[key]}%`}</Typography>
                                                        </Box>
                                                    </Box>
                                            ...
                                            {/* <SpeedDial
                                                ...
                                                icon={<SpeedDialIcon icon={<MoreVert />} openIcon={<KeyboardArrowDown />} />}
                                                FabProps={{ size: "small" }}
                                                hidden={hidden.history[key]}
                                                direction="up"
                                                open={openDial.history[key]}
                                                onOpen={() => handleOpenDial("history",key,true)}
                                                onClose={() => handleOpenDial("history",key,false)}
                                            >
                                                <SpeedDialAction
                                                    key="Rename"
                                                    icon={<EditAttributes />}
                                                    tooltipTitle="Rename"
                                                    onClick={() => editLabel("history",key,item.link,item.label)}
                                                />
                                            </SpeedDial> */}
                                        </Box>
                                    </Grid>}
            ...
            <Modal open={openView} onClose={closeForm} aria-labelledby="simple-modal-title" aria-describedby="simple-modal-description" onKeyDown={(e) => keyPress(e,selView.content)}>
                <Card className={classes.card}>
                    ...
                    <Carousel navButtonsAlwaysVisible={true} fullHeightHover={false} height="80vh" autoPlay={false} indicators={false} prev={() => prevFile(selView.content)} next={() => nextFile(selView.content)}>
                    ...
                    {(selView.content == "history" && progHistory[selView.key] == 1) &&
                    <Box class={classes.pbar1}>
                        <CircularProgress variant="determinate" color="secondary" size={68} value={barHistory[selView.key]} />
                        <Box top={0} left={0} bottom={0} right={0} alignItems="center" className={classes.pbar2}>
                            <Typography variant="body2" style={{color:'#ffffff'}}>{`${barHistory[selView.key]}%`}</Typography>
                        </Box>
                    </Box>}
                    {selView.blob instanceof Blob &&
                    <CardActions>
            ...
            <StockContainerAddAttachmentForm open={openAttach} data={container} field={field} replace={replace} closeModal={closeAttachment} dataRefresh={containerSwr?.mutate} alert={(msg) => showToast(msg)} />
