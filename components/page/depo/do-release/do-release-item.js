...
export default function DoReleaseItem(props) {
    ...
    function openDetail(e) {
        if(!(props?.active)) {
            handleMenuClose()
            e.preventDefault()
            router.push(getRoute("depo.do.release.detail", {depoSlug: depoSlug, id: props?.data?.id}))
        }
    }
    ...
    return (
        <div className={"stock-container-item " + (props?.active ? "active" : "")}>
            <Link 
                href={!(props?.active) ? getRoute("depo.do.release.detail", {depoSlug: depoSlug, id: props?.data?.id}) : "#"} 
