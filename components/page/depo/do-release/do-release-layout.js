...
import DoReleaseItem from "./do-release-item";
...
export default function DoReleaseLayout(props) {
    ...
    const [doReleases, setDoRelease] = useState([])
    var productSwr = getListDOReleaseSwr({
        search: search, 
        page: page, 
        limit: 20,
        orderBy: 'name',
    ...
    useEffect(() => {
        ...
        if(productSwr?.data?.result) {
            if(page == 1) {
                ...
                setDoRelease(productSwr?.data?.result)
                setMaxData(until == total)
            } else {
                var until = productSwr?.data?.until
                var tempList = doReleases
                productSwr?.data?.result?.forEach((item, index) => {
                    var find = doReleases?.findIndex((val) => val?.id == item?.id)
                    if(find < 0) tempList.push(item)
                })
                setTotal(productSwr?.data?.total)
                setDoRelease(tempList)
                setMaxData(until == total)
            }
        }
    }, [productSwr])
    ...
    const sideBar = (
        <div className="sidebar-wrapper">
            ...
            {!doReleases && "Loading"}
            {doReleases && <div className="sidebar-content list pb-5">
                {doReleases?.map((val, i) => {
                    return <DoReleaseItem 
                        key={val?.id} 
                        data={val}/>
                })}
                {isLoading && <Grid
