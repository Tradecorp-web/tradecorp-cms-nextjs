...
export default function DriverLayout(props) {
    ...
    var driverSwr = getListDriverSwr({
        search: search, 
        page: page, 
        limit: 20,
        orderBy: 'name',
    ...
    useEffect(() => {
        setLoading(driverSwr?.isLoading)
        if(driverSwr?.data?.result) {
            if(page == 1) {
                var until = driverSwr?.data?.until
                setTotal(driverSwr?.data?.total)
                setDrivers(driverSwr?.data?.result)
                setMaxData(until == total)
            } else {
                var until = driverSwr?.data?.until
                var tempList = drivers
                driverSwr?.data?.result?.forEach((item, index) => {
                ...
                <Grid item md={4} lg={2}>
                    <div className="sidebar-wrapper">
                        ...
                        {drivers && <div className="sidebar-content list pb-5">
                            {drivers?.map((val, i) => {
                                return <DriverItem key={val?.id} data={val}/>
                            })}
                            {isLoading && <Grid
                                container
                                spacing={0}
