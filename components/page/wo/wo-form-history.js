...
import { getWOHistorySwr } from "../../../services/swr/wo-history.swr";
...
export default function WOForm(props) {
  ...
  var woStatusSwr = getWOHistorySwr(woNumber);

  useEffect(() => {
    if (props?.open) {
      if (props?.isEdit) {
        setWoNumber(props?.data?.wo_number);
      }
    }
  }, [props?.open]);

  useEffect(() => {
    setLoading(woStatusSwr?.isLoading);

    if (woStatusSwr?.data?.result) {
      setTotalPoStatus(woStatusSwr?.data?.total);
      setPoStatusList(woStatusSwr?.data?.result);
    }
  }, [woStatusSwr]);
      ...
      <div className="modal-wrapper" style={{ width: "80%" }}>
        <Card className="modal">
          <div className="modal-header">
            <h3>{props?.isEdit ? "WO History" : "WO History"}</h3>
          </div>
          <div className="modal-content">
            <TableContainer component={Card}>
              <Table aria-label="simple table">
                <TableHead>
                  <TableRow>
                    <TableCell colSpan={3}>WO Number : {woNumber}</TableCell>
                  </TableRow>
                  <TableRow>
                    ...
                    poStatusList.map((data, index) => {
                      return (
                        <TableRow key={index} hover={true}>
                          <TableCell>
                            {moment(data?.date_process).format(
