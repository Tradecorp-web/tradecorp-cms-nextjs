...
import { dateExpired, urlPhoto } from "../../../helpers/general";

export default function DriverJobItem(props) {

    const router = useRouter()
    const driverId = router.query.id
    const urlDetail = getRoute("driver.job.detail", {id: props?.data?.id})
        ...
        <div className={"stock-container-item " + (isActive ? "active" : "")}>
            <Link 
                ...
                <div className="flex-center">
                    <Avatar alt={props?.data?.driver?.user?.name} src={urlPhoto(props?.data?.driver?.user?.photo)} style={{width: "40px", height: "40px"}} />
                    <div className="ms-2">
                        <div>
