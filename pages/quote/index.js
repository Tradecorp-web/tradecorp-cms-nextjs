...
const PageLayout = dynamic(() => import('../../components/page/quote/list-page'), {
    loading: () => (
      <Loading />
    ),
    ssr: false,
...
export default function Page() {
    ...
    useEffect(() => {
        ...
        if(res) {
            if (!isPermit("menu","quote")) {
                localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error")
                localStorage.setItem(LOCAL_STORAGE_MESSAGE, "You don't have permission on Quote Menu")
                router.push('/')
            }
        } else {
