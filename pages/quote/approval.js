...
const PageLayout = dynamic(() => import('../../components/page/quote/list-approval'), {
    loading: () => (
      <Loading />
    ),
    ssr: false,
...
export default function Page() {
    ...
    useEffect(() => {
        ...
        if(res) {
            if (!isPermit("menu","quote-approve")) {
                localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error")
                localStorage.setItem(LOCAL_STORAGE_MESSAGE, "You don't have permission on Quotation Approval Menu")
                router.push('/')
            }
        } else {
