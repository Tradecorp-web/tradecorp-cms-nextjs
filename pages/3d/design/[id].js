...
const PageLayout = dynamic(() => import('../../../components/page/3d/design-page'), {
    loading: () => (
      <Loading />
    ),
    ssr: false,
    ...
    useEffect(() => {
        ...
        if(res) {
            if (!isPermit("menu","3d")) {
                ...
                localStorage.setItem(LOCAL_STORAGE_MESSAGE, "You don't have permission on 3D Menu")
                router.push('/')
            }
        } else {
