...
const PageLayout = dynamic(
  () =>
    import(
      "../../../../components/page/accounting/setting/acc-bank-statement-setup/input"
    ),
  {
    loading: () => <Loading />,
    ssr: false,
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(
          getRoute("accounting.setting.acc-bank-statement-setup.input")
        );
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on Acc Payment Setup Menu"
        );
        router.push("/");
      }
    } else {
