...
const PageLayout = dynamic(
  () => import("../../../../components/page/accounting/report/trial-balance"),
  {
    loading: () => <Loading />,
    ssr: false,
  }
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(getRoute("accounting.report.trialbalance"));
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on Cash Flow Report Menu"
        );
        router.push("/");
      }
    } else {
