...
const PageLayout = dynamic(
  () => import("../../../../components/page/accounting/report/general-ledger"),
  {
    loading: () => <Loading />,
    ssr: false,
  }
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(getRoute("accounting.report.generalledger"));
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on General Ledger Report Menu"
        );
        router.push("/");
      }
    } else {
