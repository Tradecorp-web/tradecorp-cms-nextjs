...
const PageLayout = dynamic(
  () =>
    import(
      "../../../../components/page/accounting/transaction/purchase/vendor"
    ),
  {
    loading: () => <Loading />,
    ssr: false,
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(getRoute("accounting.transaction.purchase.vendor"));
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on Vendor  Menu"
        );
        router.push("/");
      }
    } else {
