...
const PageLayout = dynamic(
  () =>
    import(
      "../../../../components/page/accounting/transaction/assets-depreciation"
    ),
  {
    loading: () => <Loading />,
    ssr: false,
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(getRoute("accounting.transaction.assetsdepreciation"));
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on  Asset Depreciation Menu"
        );
        router.push("/");
      }
    } else {
