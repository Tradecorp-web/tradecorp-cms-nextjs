...
const PageLayout = dynamic(
  () =>
    import(
      "../../../components/page/accounting/transaction/transaction-landing"
    ),
  {
    loading: () => <Loading />,
    ssr: false,
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(getRoute("accounting.transaction-landing"));
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on Transaction  Menu"
        );
        router.push("/");
      }
    } else {
