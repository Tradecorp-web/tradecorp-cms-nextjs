...
const PageLayout = dynamic(
  () =>
    import("../../../../components/page/accounting/transaction/amortization"),
  {
    loading: () => <Loading />,
    ssr: false,
  }
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(getRoute("accounting.transaction.amortization"));
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on Transaction Amortization Menu"
        );
        router.push("/");
      }
    } else {
