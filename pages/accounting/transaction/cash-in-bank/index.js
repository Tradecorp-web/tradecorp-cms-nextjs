...
const PageLayout = dynamic(
  () =>
    import("../../../../components/page/accounting/transaction/cash-in-bank"),
  {
    loading: () => <Loading />,
    ssr: false,
  }
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(getRoute("accounting.transaction.cashinbank"));
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on Cash in Bank Menu"
        );
        router.push("/");
      }
    } else {
