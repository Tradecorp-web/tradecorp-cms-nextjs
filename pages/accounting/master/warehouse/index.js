...
const PageLayout = dynamic(
  () => import("../../../../components/page/accounting/master/warehouse"),
  {
    loading: () => <Loading />,
    ssr: false,
  }
  ...
  useEffect(() => {
    ...
    if (res) {
      if (isPermit("menu", "accounting")) {
        router.push(getRoute("accounting.master.warehouse"));
      } else {
        localStorage.setItem(LOCAL_STORAGE_MESSAGE_TYPE, "error");
        localStorage.setItem(
          LOCAL_STORAGE_MESSAGE,
          "You don't have permission on Chart of Warehouse Menu"
        );
        router.push("/");
      }
    } else {
