...
const PageLayout = dynamic(() => import('../../components/page/files/list-deleted'), {
    loading: () => (
      <Loading />
    ),
    ssr: false,
    ...
    useEffect(() => {
        ...
        if(res) {
            if (!isPermit("menu","files")) {
                ...
                localStorage.setItem(LOCAL_STORAGE_MESSAGE, "You don't have permission on My Files Menu")
                router.push('/')
            }
        } else {
