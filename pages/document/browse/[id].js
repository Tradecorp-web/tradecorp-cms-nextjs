...
const PageLayout = dynamic(() => import('../../../components/page/document/list-document'), {
    loading: () => (
      <Loading />
    ),
    ssr: false,
    ...
    useEffect(() => {
        ...
        if(res) {
            if (!isPermit("menu","document")) {
                ...
                localStorage.setItem(LOCAL_STORAGE_MESSAGE, "You don't have permission on Authorized Document for Distribution Menu")
                router.push('/')
            }
        } else {
