...
const PageLayout = dynamic(() => import('../../components/page/task/list-task'), {
    loading: () => (
      <Loading />
    ),
    ssr: false,
    ...
    useEffect(() => {
        ...
        if(res) {
            if (!isPermit("menu","task_manager")) {
                ...
                localStorage.setItem(LOCAL_STORAGE_MESSAGE, "You don't have permission on Task Manager Menu")
                router.push('/')
            }
        } else {
